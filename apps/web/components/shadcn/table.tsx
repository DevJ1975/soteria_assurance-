...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn('border-b border-border-soft transition-colors hover:bg-muted/60', className)}
      {...props}
    />
  ),
);
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      'h-11 px-4 text-left align-middle text-xs font-semibold uppercase tracking-wide text-text-faint [&:has([role=checkbox])]:pr-0',
      className,
    )}
    {...props}
  />
